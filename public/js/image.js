    ...
    function draw_box_on_image(ctx, box, box_corners, trans_ratio, selected){
        ...
        if (!selected){
                ...
                if (box.world.data.cfg.color_obj == "category")
                        target_color = globalObjectCategory.get_color_by_category(box.obj_type,box.world.frameInfo.scene.substring(0,4) == 'nusc');
                else // by id
                {
...
class ImageContext extends MovableView{
    ...
    on_click(e) {
        ...
        if (!this.drawing && e.ctrlKey){
                ...
                var c = this.ui;
                c.onmousemove = this.on_move;
                c.ondblclick = this.on_dblclick;
                c.onkeydown = this.on_key;
        }
        else {
    ...
    on_move(e){
        let p= this.to_viewbox_coord(e.layerX, e.layerY);
        var x=p[0];
        var y=p[1];
        console.log(x,y);
