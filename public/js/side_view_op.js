...
class ProjectiveView{
    ...
    update_view_handle(viewport, obj_dimension, obj_pos, obj_rot){
        ...
        if (this.on_direction_changed){
            ...
            de.setAttribute('y', 0);//Math.ceil(top+10));    
            de.setAttribute('height', Math.max(Math.ceil((bottom-top)/2-10+top),Number.EPSILON));
        }
        else{
            de = this.lines.direction;
        ...
        if(event.altKey) {
            switch (event.key) {
                case 'e':
                    event.preventDefault();
                    event.stopPropagation();
                    this.on_direction_changed(-this.cfg.rotateStep, false);
...
class ProjectiveViewOps{
    ...
    update_view_handle(){
        if (this.box){
            ...
            // this.z_view_handle.update_view_handle(this.views[0].getViewPort(), {x: this.box.scale.y, y:this.box.scale.x});
