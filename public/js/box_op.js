...
function BoxOp(){
    ...
    this.interpolateAsync = async function(worldList, boxList, applyIndList){
        ...
        for (let i = 0; i< boxList.length; i++){
            ...
            if (!boxList[i]){
                ...
                let globalpos = boxList[i].world.lidarPosToUtm(b.position);
                let globalrot = boxList[i].world.lidarRotToUtm(b.rotation);
    ...
    this.copyGlobalPos = async function(boxworld,box, onFinishOneBoxCB, applyworld) {
        if (!box)
            return null;
        let anns = boxworld.annotation.ann_to_utm_vector(box);
        let obj_type = box.obj_type;
        let obj_track_id = box.obj_track_id;
        let obj_attr = box.obj_attr;
        let applyworld_box = applyworld.annotation.findBoxByTrackId(obj_track_id);

        if (!applyworld_box){
            // create new box
            let ann = applyworld.annotation.utm_vector_to_ann(anns);
            let newBox  = applyworld.annotation.add_box(ann.position,
                ann.scale,
                ann.rotation,
                obj_type,
                obj_track_id,
                obj_attr);
            newBox.annotator="c";
            applyworld.annotation.load_box(newBox);
        }
        else if (applyworld_box.annotator) {
            // modify box attributes
            let b = applyworld.annotation.utm_vector_to_ann(anns);
            applyworld_box.position.x = b.position.x;
            applyworld_box.position.y = b.position.y;
            applyworld_box.position.z = b.position.z;

            applyworld_box.scale.x = b.scale.x;
            applyworld_box.scale.y = b.scale.y;
            applyworld_box.scale.z = b.scale.z;

            applyworld_box.rotation.x = b.rotation.x;
            applyworld_box.rotation.y = b.rotation.y;
            applyworld_box.rotation.z = b.rotation.z;
