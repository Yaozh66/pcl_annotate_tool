...
class FastToolBox{
    ...
    {
        let attrs = ["static"];
        let sceneName = document.querySelector("#scene-input").value;
        if(sceneName.substring(0,4) == 'nusc')
            if (globalObjectCategory.nusc_obj_type_map[obj_type] && globalObjectCategory.nusc_obj_type_map[obj_type].attr)
                attrs = attrs.concat(globalObjectCategory.nusc_obj_type_map[obj_type].attr);
        else
