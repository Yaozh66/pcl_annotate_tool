    ...
    */
    ...
    this.updateInfo = function(){
        ...
        if (this.target.world){
            info  += String(this.target.world.frameInfo.frame_index+1);
...
function BoxEditorManager(parentUi, viewManager, objectTrackView, 
    ...
    this.updateAutoGeneratedBoxes = function(){
        ...
        500);
        this.editorList.map(e=>e.box).forEach(b=>{
            if(b){
                let globalpos = b.world.lidarPosToUtm(b.position);
                let globalrot = b.world.lidarRotToUtm(b.rotation);
                b.globalpsr = {position:{x:globalpos.x,y:globalpos.y,z:globalpos.z},
                    rotation:{x:globalrot.x,y:globalrot.y,z:globalrot.z}};
                window.editor.change_box_velocity(b);
                window.editor.tracker.update_box(b.world.frameInfo.frame,b,"modify");
            }
        });
    };
    ...
    this.TrackSelectedFrames =async function () {
        let selected_frame_index = this.getSelectedEditors().map(e=>e.target.world.frameInfo.frame_index);
        let finishCB = ()=>{
            this.getSelectedEditors().forEach(e=>{
                e.tryAttach();
                this.viewManager.render();
        ...
        await window.editor.tracker.track(this.editingTarget.data.world.frameInfo.scene,-1,true,selected_frame_index,this.editingTarget.objTrackId,finishCB);
    }
    ...
    this.handleContextMenuEvent =async function(event)
    {
        ...
        case 'cm-delete-intersected-boxes':
            ...
            break;
        case 'cm-track':
            await this.TrackSelectedFrames();
            break;
        case 'cm-interpolate':
            this.interpolateSelectedFrames();
            break;
    ...
    this.reloadAnnotation = function(editorList){
        ...
        if (modifiedFrames.length > 0) {
            window.editor.infoBox.show(
                ...
                (choice)=>{
                    if (choice=="yes")
                        reloadWorldList(worldList, done,editorList.map(e=>e.target.objTrackId)[0]);
                }
            );
        }
        else
    ...
    this.showTrajectory = () =>{
        let tracks = this.editingTarget.data.worldList.map(w=>{
            ...
            if (box){
                ...
                ann.psr.position = w.globalpsr.position;
                ann.psr.rotation = w.globalpsr.rotation;
            } 
    ...
    this.toolbox.querySelector("#auto-track").onclick = async ()=>{
        await this.TrackSelectedFrames();
    };
