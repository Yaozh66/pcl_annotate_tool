...
PCDLoader.prototype = {
	...
	parseBin: function(data, url){
		...
		var color = [];
		var intensity = [];
		//kitti format:xyzi, nusc format: xyzif
		var offset = 0;
		var each_size=16;
		if(url.includes("v1.0"))
			each_size=20

		for ( var row = 0; row < data.byteLength/each_size; row += 1 ) {
			position.push( dataview.getFloat32( row*each_size + 0, this.littleEndian ) );
			position.push( dataview.getFloat32( row*each_size + 4, this.littleEndian ) );
			position.push( dataview.getFloat32( row*each_size + 8, this.littleEndian ) );
			intensity.push(dataview.getFloat32( row*each_size + 12, this.littleEndian ) )
		}
