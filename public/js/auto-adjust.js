...
function AutoAdjust(boxOp, mouse, header){
    ...
    this.mark_bbox=function(box){
        if (box){
            ...
            this.marked_object =null;
            this.header.refObjUi.innerHTML = ''
        }
    };
    this.followStaticObjects = function(box) {
        ...
        worldList.forEach(w=>{
            ...
            if (existedBox){
                ...
                let globalpos = existedBox.world.lidarPosToUtm(existedBox.position);
                let globalrot = existedBox.world.lidarRotToUtm(existedBox.rotation);
                existedBox.globalpsr = {position:{x:globalpos.x,y:globalpos.y,z:globalpos.z},
    ...
    this.followsRef = function(box){
        ...
        if (refObj){
            ...
            worldList.forEach(w=>{
                ...
                if (existedBox && !existedBox.annotator)
                    // have same objects annotated.
                    // if it's generated by machine, let's overwrite it
                    return;
                ...
                console.log("modified or added box in ", w.frameInfo.frame);
                w.annotation.setModified();
            });
        }
    };
    ...
    this.syncFollowers = function(box){
        ...
        allFollowers.forEach(fb=>{
            ...
            let globalpos = fb.world.lidarPosToUtm(fb.position);
            let globalrot = fb.world.lidarRotToUtm(fb.rotation);
            fb.globalpsr = {position:{x:globalpos.x,y:globalpos.y,z:globalpos.z},
                rotation:{x:globalrot.x,y:globalrot.y,z:globalrot.z}};
