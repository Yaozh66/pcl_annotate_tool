    ...
    this.preload = function(on_preload_finished){
        ...
        loader.load( this.frameInfo.get_radar_path(this.name), 
            ...
            function ( pcd ) {
                ...
                var velocity = position.map((p,i)=>pcd.velocity[i]*_self.velocityScale+pcd.position[i]);
    ...
    this.createRadarBox = function(){
        if (this.sceneMeta.calib.radar && this.sceneMeta.calib.radar[this.name]){
            return this.world.annotation.createCuboid(
                ...
                {x:0.5,y:0.5, z:0.5},
                {
                    x: this.sceneMeta.calib.radar[this.name].rotation[2],
                    y: this.sceneMeta.calib.radar[this.name].rotation[1],
                    z: this.sceneMeta.calib.radar[this.name].rotation[0],
                }, 
                "radar", 
                this.name,null,true);
...
function RadarManager(sceneMeta, world, frameInfo){
    ...
    this.showPoints = function (){
        this.radarList.forEach(r=>r.showPoints());
    }
    this.hidePoints = function (){
        this.radarList.forEach(r=>r.hidePoints());
    }
