...
from tools.my_nuscenes.utils.data_classes import LidarPointCloud, RadarPointCloud, Box
from tools.my_nuscenes.utils.geometry_utils import view_points, box_in_image, BoxVisibility, transform_matrix
from tools.my_nuscenes.utils.map_mask import MapMask
from tools.my_nuscenes.utils.color_map import get_colormap
...
class NuScenes:
    ...
    def get_box(self, sample_annotation_token: str) -> Box:
        ...
        return Box(record['translation'], record['size'], Quaternion(record['rotation']),
                   name=record['category_name'], token=record['token'], instance_token=record["instance_token"])
    ...
    def get_boxes(self, sample_data_token: str) -> List[Box]:
        ...
        """
        # Regularly the sample data token corresponds to lidar token
        # Retrieve sensor & pose records
        sd_record = self.get('sample_data', sample_data_token)
            ...
            for curr_ann_rec in curr_ann_recs:
                ...
                if curr_ann_rec['instance_token'] in prev_inst_map:
                    ...
                    box = Box(center, curr_ann_rec['size'], rotation, name=curr_ann_rec['category_name'],
                              token=curr_ann_rec['token'],instance_token=curr_ann_rec['instance_token'])
                else:
